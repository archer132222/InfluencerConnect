...
export default function Login() {
  ...
  const handleLogin = (role: 'influencer' | 'customer') => {
    login(role);
    // Wait for the store to update (mock delay handled in store, but here we redirect based on role)
    setTimeout(() => {
      ...
      <div className="container mx-auto px-4 py-16 flex justify-center items-center min-h-[calc(100vh-64px)]">
        <Card className="w-full max-w-md bg-[#1a1a1a] border-white/10 text-white">
          <CardHeader className="text-center space-y-2">
            ...
            <p className="text-sm text-gray-400">Welcome back to Influencer Connect</p>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "As Brand"}
              </Button>
              <Button 
              ...
              >
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "As Influencer"}
              </Button>
            </div>
